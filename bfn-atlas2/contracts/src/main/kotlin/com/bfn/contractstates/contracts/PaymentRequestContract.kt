...
class PaymentRequestContract : Contract {
    ...
    override fun verify(tx: LedgerTransaction) {
        logger.info(
                "\uD83D\uDD06 PaymentRequestContract: verify starting ..... \uD83E\uDD6C \uD83E\uDD6C ")

        val cmd = tx.commands.first()
        if (cmd.value is Register) {
            if (tx.outputStates.isEmpty()) {
                throw IllegalArgumentException("There should be an output state of PaymentRequest")
            }
...
//            }
    ...
    companion object {
        val ID: String = PaymentRequestContract::class.java.name
        private val logger = LoggerFactory.getLogger(PaymentRequestContract::class.java)
    }
}
